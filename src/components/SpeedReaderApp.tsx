...
const PG_ESSAYS = [
  "greatwork", "kids", "selfindulgence", "lies", "re", "mean", "hs", "vb", "brandage", "field", 
  "goodwriting", "do", "woke", "writes", "when", "foundermode", "persistence", "reddits", "google", 
  "best", "superlinear", "getideas", "read", "want", "alien", "users", "heresy", "words", "goodtaste", 
  "smart", "weird", "hwh", "own", "fn", "newideas", "nft", "real", "richnow", "simply", "donate", 
  "worked", "earnest", "ace", "airbnbs", "think", "early", "wtax", "conformism", "orth", "cred", 
  "useful", "noob", "fh", "mod", "fp", "lesson", "nov", "genius", "sun", "pow", "disc", "pgh", 
  "ineq", "jessica", "bias", "talk", "aord", "safe", "name", "altair", "ronco", "work", "corpdev", 
  "95", "ecw", "know", "pinch", "before", "fr", "herd", "convince", "ds", "invtrend", "startupideas", 
  "hw", "growth", "swan", "todo", "speak", "ycstart", "property", "ambitious", "word", "schlep", "vw", 
  "hubs", "patentpledge", "airbnb", "control", "tablets", "founders", "superangels", "seesv", 
  "hiresfund", "yahoo", "future", "addiction", "top", "organic", "apple", "really", "discover", 
  "publishing", "nthings", "determination", "kate", "segway", "ramenprofitable", "makersschedule", 
  "revolution", "twitter", "foundervisa", "5founders", "relres", "angelinvesting", "convergence", 
  "maybe", "hackernews", "13sentences", "identity", "credentials", "divergence", "highres", 
  "artistsship", "badeconomy", "fundraising", "prcmc", "cities", "distraction", "good", "googles", 
  "heroes", "disagree", "boss", "ycombinator", "trolls", "newthings", "startuphubs", "webstartups", 
  "philosophy", "colleges", "die", "head", "stuff", "equity", "unions", "guidetoinvestors", 
  "judgement", "microsoft", "notnot", "wisdom", "foundersatwork", "goodart", "startupmistakes", 
  "mit", "investors", "copy", "island", "marginal", "america", "siliconvalley", "startuplessons", 
  "randomness", "softwarepatents", "6631327", "whyyc", "love", "procrastination", "web20", 
  "startupfunding", "vcsqueeze", "ideas", "sfp", "inequality", "ladder", "opensource", "hiring", 
  "submarine", "bronze", "mac", "writing44", "college", "venturecapital", "start", "usa", "charisma", 
  "polls", "laundry", "bubble", "essay", "pypar", "gh", "gap", "wealth", "gba", "say", "ffb", "hp", 
  "iflisp", "hundred", "nerds", "better", "desres", "spam", "icad", "power", "fix", "taste", "noop", 
  "diff", "road", "rootsoflisp", "langdes", "popular", "javacover", "avg", "lwba", "progbot", "prop62", "rss"
];
...
export default function SpeedReaderApp() {
  ...
  const [animations, setAnimations] = useState<{ id: string; type: 'FASTER' | 'SLOWER' }[]>([]);
  ...
  }, [appState, pause, togglePause, adjustWpm, rewind, skipForward, isPlaying, wpm]);
  ...
  const handleFetchUrl = async () => {
    ...
    try {
      ...
      if (formattedUrl.startsWith('http://')) formattedUrl = formattedUrl.slice(7);
      if (formattedUrl.startsWith('https://')) formattedUrl = formattedUrl.slice(8);
  ...
  if (appState === 'INPUT') {
    return (
      <div className="w-full flex flex-col gap-6 max-w-2xl mx-auto mt-12 sm:mt-24">
        <h1 className="text-xl font-bold uppercase tracking-widest text-gray-500">RSVP Speed Reader</h1>
        <textarea
          className="w-full h-64 p-4 border border-gray-300 dark:border-gray-700 bg-transparent resize-none font-mono text-sm focus:outline-none focus:border-black dark:focus:border-white transition-colors"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
          ...
          <div className="flex flex-col items-end gap-2 w-full sm:w-auto">
            ...
            {errorMsg && <span className="text-red-500 font-mono text-xs">{errorMsg}</span>}
          </div>
        </div>
        
        <div className="mt-8 pt-8 border-t border-gray-200 dark:border-gray-800">
          <h2 className="text-sm font-mono text-gray-500 mb-4 uppercase tracking-widest">Try an Example:</h2>
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button 
              onClick={() => {
                const randomSlug = PG_ESSAYS[Math.floor(Math.random() * PG_ESSAYS.length)];
    ...
    return (
      <div className="w-full max-w-2xl mx-auto mt-12 pb-24">
        <div className="mb-8 flex justify-between items-center border-b border-gray-200 dark:border-gray-800 pb-4">
          <span className="font-mono text-sm text-gray-500">Full Text Mode</span>
          <button 
  ...
  return (
    <div 
      className={`w-full h-[90vh] flex flex-col justify-between relative overflow-hidden rounded-2xl ${
    ...
    >
      ...
      {animations.filter(a => a.type === 'FASTER').map((anim) => (
        <div key={anim.id} className="absolute top-0 right-0 bottom-0 w-1/3 pointer-events-none z-0 flex items-center justify-end pr-8 sm:pr-16 mix-blend-screen dark:mix-blend-lighten">
      ...
      {animations.filter(a => a.type === 'SLOWER').map((anim) => (
        <div key={anim.id} className="absolute top-0 left-0 bottom-0 w-1/3 pointer-events-none z-0 flex items-center justify-start pl-8 sm:pl-16 mix-blend-screen dark:mix-blend-lighten">
      ...
      <div className="w-full flex justify-between items-start font-mono text-xs sm:text-sm text-gray-500 uppercase tracking-widest border-b border-gray-200 dark:border-gray-800 pb-4 max-w-2xl mx-auto z-10">
        ...
        <div className="flex-1 text-center font-bold">{isPlaying ? 'PLAYING' : 'PAUSED'}</div>
        <div className="w-32 text-right">Word: {Math.min(currentIndex + 1, words.length)} / {words.length}</div>
      </div>
      ...
      <div className="w-full max-w-2xl mx-auto mt-auto pt-4 border-t border-gray-200 dark:border-gray-800 z-10">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-3 text-xs font-mono text-gray-500">
          <div><b className="text-black dark:text-white">SPACE</b> Increase speed</div>
          <div><b className="text-black dark:text-white">CTRL+SPACE</b> Decrease speed</div>
          <div><b className="text-black dark:text-white">P</b> Pause / Resume</div>
          <div><b className="text-black dark:text-white">← / →</b> Rewind / Skip</div>
          <div><b className="text-black dark:text-white">F</b> Full Text Mode</div>
          <div><b className="text-black dark:text-white">ESC</b> Exit session</div>
        </div>
        <div className="mt-4 w-full h-1 bg-gray-100 dark:bg-gray-900 rounded-full overflow-hidden">
          <div 
            ...
            style={{ width: `${(currentIndex / Math.max(1, words.length - 1)) * 100}%` }}
          />
        </div>
      </div>
    </div>
